#!/usr/bin/env bash
# ViT-Base/16 on ImageNet-1k, AdamW baseline (Fig. 10d configuration).
...
SEED="${SEED:-0}"
NGPUS="${NGPUS:-3}"
torchrun --nproc_per_node="$NGPUS" scripts/train.py --distributed --dataset imagenet --data_dir "$DATA_DIR" --model vit_base --optimizer adamw --lr 0.0015 --weight_decay 0.05 --train_batch_size 600 --test_batch_size 128 --train_epochs 200 --cosine_lr_sched --lr_warmup_epochs 15 --lr_min 1e-6 --num_workers 16 --random_seed "$SEED" --trainer base --log_dir "$LOG_ROOT/imagenet/vit_base_baseline_seed$SEED" "$@"
