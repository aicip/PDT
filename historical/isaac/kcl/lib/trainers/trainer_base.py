...
class TrainerBase:
    ...
    def get_lr_scheduler(self, optimizer):
        if self.args.cosine_lr_sched:
            ...
            if self.args.two_stage_training:  # add the support for two stage scheduler
            # first stage scheduler
                first_stage_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                    optimizer,
                    T_max=self.args.first_stage_epochs - self.args.lr_warmup_epochs,
                    eta_min=self.args.lr_min
                )

                # second stage scheduler: use CosineAnnealingLR directly with higher starting LR
                class SecondStageLR(torch.optim.lr_scheduler.CosineAnnealingLR):
                    def __init__(self, optimizer, T_max, eta_min):
                        ...
                        # Set initial lr to 10x of min_lr
                        self.base_lrs = [self.eta_min * 10 for _ in self.base_lrs]

                second_stage_scheduler = SecondStageLR(
                    optimizer,
                    T_max=self.train_epochs - self.args.first_stage_epochs,
                    eta_min=self.args.lr_min
                )
                ...
                if self.args.lr_warmup_epochs > 0:
                    ...
                    schedulers = [warmup_scheduler, first_stage_scheduler, second_stage_scheduler]
                    milestones = [self.args.lr_warmup_epochs, self.args.first_stage_epochs]
                else:
                    schedulers = [first_stage_scheduler, second_stage_scheduler]
                    milestones = [self.args.first_stage_epochs]
    ...
    def train_epoch(self, train_loader, loss_func):
        ...
        do_profile = self.args.count_flops and self.batch_forward_flops is None and self.args.dist_rank_0  # only profile on first batch, only on rank 0
        do_profile_fast = self.args.count_flops_fast and self.batch_forward_flops is None and self.args.dist_rank_0  # only profile on first batch for fast mode, only on rank 0
        ...
        for batch_idx, (batch) in enumerate(train_loader):
            if do_profile and batch_idx == 0:
                ...
                # Timing profiling: batch start (measure first 100 batches for averaging, only on rank 0)
                ...
                # Timing profiling: batch end (only on rank 0)
                if self.args.profile_timing and self.args.dist_rank_0 and batch_idx < 100:
                    torch.cuda.synchronize() if torch.cuda.is_available() else None
        ...
        # Timing profiling: epoch end (only on rank 0, must match epoch start check)
        if self.args.profile_timing and self.args.dist_rank_0:
            torch.cuda.synchronize() if torch.cuda.is_available() else None
