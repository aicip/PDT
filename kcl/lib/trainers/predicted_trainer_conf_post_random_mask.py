...
#        raise Exception("Stopping the program for inspection")
    ...
    def _perform_dmd_multistep_prediction_and_update(self,epoch_gradient):
        ...
        if self.weights is not None:
            ...
            #mask_seed = 5  # Set this to any desired value, or generate it randomly for each run
            mask_seed = int(time.time())  # Use the current time as the seed for the mask
            mask_generator.manual_seed(mask_seed)
            mask = torch.rand(current_weights.size(), generator=mask_generator, dtype=torch.float32, device=current_weights.device) < 0.09699   # set the mask ratio
