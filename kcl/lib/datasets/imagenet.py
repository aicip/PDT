...
from kcl.lib.datasets.transforms_torchvision import (
...
def create_imagenet_sota(
    ...
    rank=0,
    ws=1,
    # SOTA training recipe parameters
    auto_augment='ra',  # 'ra' for RandAugment, 'ta_wide' for TrivialAugmentWide, 'imagenet' for AutoAugment
    ra_magnitude=9,
    ...
    val_crop_size=224,
    val_resize_size=256,
    use_ra_sampler=False,  # Whether to use RASampler (Repeated Augmentation Sampler)
):
    """
    Create ImageNet dataloaders with TorchVision SOTA training recipe.

    This function implements the data loading pipeline used in TorchVision's
    reference training scripts that achieve SOTA accuracy:
    - ResNet-50 V2: 80.858% with auto_augment='ra', mixup_alpha=0.2, cutmix_alpha=1.0
    - ViT-Base/16: 81.072% with auto_augment='ra', mixup_alpha=0.2, cutmix_alpha=1.0

    Args:
        dataset_dir: Path to ImageNet dataset
        train_batch_size: Per-GPU batch size for training
        test_batch_size: Per-GPU batch size for testing
        distributed: Whether using distributed training
        rank: Process rank in distributed training
        ws: World size (number of processes)
        auto_augment: Auto-augmentation policy ('ra', 'ta_wide', 'imagenet', None)
        ra_magnitude: RandAugment magnitude (default 9)
        random_erase_prob: Probability of random erasing (0.0-1.0)
        ...
        train_crop_size: Training crop size (default 224)
        val_crop_size: Validation crop size (default 224)
        val_resize_size: Validation resize size (default 256)
        use_ra_sampler: Use Repeated Augmentation Sampler (for better accuracy)

    Returns:
        train_loader, test_loader, mixup_cutmix_transform
    """
    ...
    interp_mode = interp_map.get(interpolation.lower(), InterpolationMode.BILINEAR)

    # Create training transforms using TorchVision SOTA recipe
    train_transform = ClassificationPresetTrain(
        crop_size=train_crop_size,
        interpolation=interp_mode,
        auto_augment_policy=auto_augment,
        ra_magnitude=ra_magnitude,
        random_erase_prob=random_erase_prob,
    )

    # Create validation transforms
    val_transform = ClassificationPresetEval(
    ...
    if distributed:
        if use_ra_sampler:
            # Use Repeated Augmentation Sampler for potentially better accuracy
    ...
    # Create MixUp/CutMix transform (applied in training loop)
    ...
    return train_loader, test_loader, mixup_cutmix
